...
\begin{frame}
  ...
  \]
  \[
    \blue{H = \set{(1), (1\; 2\; 3), (1\; 3\; 2)} \le S_{3}}
  \]
...
\begin{frame}
  \begin{definition}[Quotient Group (商群)]
    ...
    \[
      aH \cdot bH = (ab)H.
    \]
    Then $(G/H, \cdot)$ is a \blue{group},
    called the \red{quotient group} of $G$ by $H$.
  \end{definition}
\end{frame}
%%%%%%%%%%%%%%%
...
\begin{frame}
  ...
  \begin{gather*}
    (1) H = H = (1\; 2) H \\[6pt]
    \teal{(1\; 3) H} = \set{(1\; 3), (1\; 2\; 3)} = \teal{(1\; 2\; 3)H} \\[6pt]
    \cyan{(2\; 3) H} = \set{(2\; 3), (1\; 3\; 2)} = \cyan{(1\; 3\; 2)H}
  \end{gather*}

  \pause
  \[
    \teal{(1\; 3) H} \cdot \cyan{(2\; 3) H} = (1\; 2\; 3)H
  \]
  \[
    \teal{(1\; 2\; 3) H} \cdot \cyan{(1\; 3\; 2) H} = H
  \]
\end{frame}
%%%%%%%%%%%%%%%
...
\begin{frame}
  ...
  \begin{align*}
    a'H \cdot b'H &= (a'b')H \\
      &= a'(b'H) = a'(b H) = \red{a' (Hb)} \\
      &= (a'H)b = (aH)b = a(Hb) = \red{a(bH)} \\
      &= (ab)H = aH \cdot bH
  \end{align*}
\end{frame}
%%%%%%%%%%%%%%%
...
\begin{frame}
  ...
  \pause
  \[
    G/H = \set{(1)H, (1\;2)H} \pause \cong \purple{\Z_{2}}
  \]
\end{frame}
%%%%%%%%%%%%%%%
...
\begin{frame}
  ...
  \[
    G/H = \pause \set{0 + H, 1 + H, \dots, 5 + H} \pause \cong \purple{\Z_{6}}
  \]
\end{frame}
%%%%%%%%%%%%%%%
...
\begin{frame}
  \begin{theorem}
    Suppose that \purple{$H \le G$}.
    \[
      H \triangleleft G
        \iff \forall a \in G\; aHa^{-1} = H
        \iff \boxed{\forall \red{a \in G}, \blue{h \in H}\; aha^{-1} \in H}
    \]
  \end{theorem}

  \pause
  \[
    \teal{\boxed{\forall a \in G\;
      (aH = Ha \to \forall h \in H\; aha^{-1} \in H)}}
  \]
  \begin{align*}
  ...
  \pause
  \[
    \teal{\boxed{\forall a \in G, h \in H\;
      (aha^{-1} \in H \to aH = Ha)}}
  \]
  \pause
  \[
